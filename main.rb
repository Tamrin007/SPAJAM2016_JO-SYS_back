...
class MainApp < Sinatra::Base
        ...
        get '/locate' do
            lat = params[:lat]
            lon = params[:lon]
            near_point = Sound.find_by_sql("SELECT file_id title comment SQRT(POWER((#{lat} - lat), 2) + POWER((#{lon} - lon), 2)) AS distance FROM sounds ORDER BY distance").limit(10).to_json
            p near_point
        end
